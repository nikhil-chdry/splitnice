...
function createDefaultShares(members) {
  return Object.fromEntries(members.map((member) => [member, 1]));
}
...
function AddExpenseModal({ isOpen, onClose, onAddExpense }) {
  ...
  // ── FIX #1: Dynamic payer based on group ───────────────────────────────
  const [payer, setPayer] = useState("You");

  const amountNumber = Number(amount) || 0;
  const amountPerPerson =
    selectedMembers.length > 0 ? amountNumber / selectedMembers.length : 0;
  ...
  const percentageTotal = selectedMembers.reduce(
    (total, member) => total + (Number(percentageAmounts[member]) || 0),
    0
  );
  const totalShares = selectedMembers.reduce(
    (total, member) => total + (Number(shareAmounts[member]) || 0),
    0
  );
  ...
  const splitIsValid =
    ...
    (splitMethod === "Equally" ||
      (splitMethod === "Exact" && Math.abs(remainingAmount) < 0.01) ||
      (splitMethod === "Percentage" && Math.abs(percentageTotal - 100) < 0.01) ||
      (splitMethod === "Shares" && totalShares > 0));

  function getMemberShare(member) {
    if (totalShares === 0) return 0;
    return (amountNumber * (Number(shareAmounts[member]) || 0)) / totalShares;
  }
  function changeShares(member, change) {
    setShareAmounts((currentShares) => {
      ...
      return { ...currentShares, [member]: nextValue };
    });
  }
  ...
  function handleGroupChange(event) {
    ...
    setShareAmounts(createDefaultShares(membersByGroup[selectedGroup]));
    // ── FIX #1: Reset payer to "You" when group changes ─────────────────
    setPayer("You");
  }
  ...
  function resetForm() {
    ...
    setExactAmounts({});
    setPercentageAmounts({});
    setShareAmounts(createDefaultShares(membersByGroup["Goa Trip"]));
    setPayer("You");
  }
  ...
  function handleSubmit(event) {
    ...
    // ── FIX #3: Build rich expense object with all split details ─────────
    let splitDetails = {};
    if (splitMethod === "Equally") {
      selectedMembers.forEach((member) => {
        splitDetails[member] = amountPerPerson;
      });
    } else if (splitMethod === "Exact") {
      selectedMembers.forEach((member) => {
        splitDetails[member] = Number(exactAmounts[member]) || 0;
      });
    } else if (splitMethod === "Percentage") {
      selectedMembers.forEach((member) => {
        splitDetails[member] = getPercentageShare(member);
      });
    } else if (splitMethod === "Shares") {
      selectedMembers.forEach((member) => {
        splitDetails[member] = getMemberShare(member);
      });
    }
    ...
    onAddExpense({
      ...
      detail: `${payer} paid ₹${formattedAmount} · ${group}`,
      time: "Just now",
      balance: payer === "You" ? "You lent" : "You owe",
      balanceColor: payer === "You" ? "text-positive" : "text-negative",
      // ── Rich data for backend later ─────────────────────────────────────
      amount: amountNumber,
      payer,
      group,
      splitMethod,
  ...
  const currentMembers = membersByGroup[group];
      ...
      >
        ...
        <form className="mt-9 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="grid gap-5 md:grid-cols-2">
            ...
            {/* ── FIX #1: Dynamic Paid by dropdown ─────────────────────── */}
            <label>
                    ...
                    >
                      ...
                      <div className="flex items-center border-b border-ink/20">
                        ...
                        <input
                          ...
                          onChange={(event) => updateExactAmount(member, event.target.value)}
                          placeholder="0.00"
          ...
          {splitMethod === "Percentage" && (
            <fieldset>
              <legend className="text-sm font-semibold">Split by percentage</legend>
              <div className="mt-3 flex justify-between rounded-2xl bg-white p-4 text-sm">
                      ...
                      <div className="text-right">
                        <div className="flex items-center border-b border-ink/20">
                          <input
                            ...
                            onChange={(event) => updatePercentage(member, event.target.value)}
                            placeholder="0"
          ...
          {splitMethod === "Shares" && (
            <fieldset>
              <legend className="text-sm font-semibold">Split by shares</legend>
              <div className="mt-3 flex justify-between rounded-2xl bg-white p-4 text-sm">
